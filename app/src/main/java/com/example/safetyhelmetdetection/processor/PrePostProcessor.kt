package com.example.safetyhelmetdetection.processor
...
object PrePostProcessor {
    ...
    lateinit var mClasses: List<String>

    // model output is of size 25200*(num_of_class+5)
    private const val mOutputRow =
        25200 // as decided by the YOLOv5 model for input image of size 640*640
    private const val mOutputColumn = 8 // left, top, right, bottom, score and 3 class probability
    ...
    fun nonMaxSuppression(
        ...
        Collections.sort(boxes,
            Comparator<DetectionObject> { o1, o2 -> o1.score.compareTo(o2.score) })
        val selected: MutableList<DetectionObject> = mutableListOf()
        ...
        while (i < boxes.size && !done) {
            if (active[i]) {
                val boxA: DetectionObject = boxes[i]
                selected.add(boxA)
                if (selected.size >= limit) break
                for (j in i + 1 until boxes.size) {
                    if (active[j]) {
                        val boxB: DetectionObject = boxes[j]
